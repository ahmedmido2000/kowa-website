  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-[#0B0F2A]/80 backdrop-blur-md shadow-lg border-b border-white/10 py-3.5"
          : "bg-transparent py-5"
      }`}
    >
      ...
      {/* Mobile Menu Drawer */}
      <div
        className={`fixed top-0 right-0 z-50 h-full w-80 bg-[#090C1E] border-l border-white/10 shadow-2xl transition-transform duration-300 ease-out lg:hidden flex flex-col ${
          ...
          <ul className="flex flex-col gap-4">
            {navLinks.map(({ label, href }) => (
              <li key={href}>
                <a
                  href={href}
                  className="block rounded-lg px-4 py-3 text-base font-medium text-white/90 hover:bg-white/10 hover:text-[#F5A623] transition-colors"
                  onClick={() => setMobileOpen(false)}
                >
                  {label}
                </a>
